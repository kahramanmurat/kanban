...
export function KanbanColumn({
  ...
  const commitTitle = () => {
    const nextTitle = draftTitle.trim() || column.title;
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 px-1">
        ...
        />
        <span className="shrink-0 rounded-full bg-[var(--navy-dark)]/5 px-2 py-0.5 text-[11px] font-semibold text-[var(--gray-text)]">
          {cards.length}
        </span>
      </div>
      <div className="mt-3 flex flex-1 flex-col gap-2">
        ...
        {cards.length === 0 && (
          <div className="flex flex-1 items-center justify-center rounded-xl border border-dashed border-[var(--stroke)] px-3 py-6 text-center text-xs text-[var(--gray-text)]">
            Drop a card here
          </div>
        )}
      </div>
